...
# Modified for Oscilloscope AE Acquisiton
# May 3, 2024 - Nick Tulshibagwale
# Overall, only minor changes from Keysight's original "brute force" script.
# Oscilloscope is manually configured to trigger on expected waveform.
# Once the oscilloscope is triggered. This script should be run in order to 
# save the oscilloscope waveform data onto the computer.
# Requires installation of VISA related drivers and PyVisa and IO suite library.
...
SCOPE_VISA_ADDRESS = "USB0::0x0957::0x1797::MY56271380::0::INSTR" # Get this from Keysight IO Libraries Connection Expert
...
rm = pyvisa.ResourceManager('C:\\Windows\\System32\\visa32.dll') # This uses PyVisa; PyVisa info @ http://PyVisa.readthedocs.io/en/stable/
...
if CHAN1_STATE == 1:
    ...
    ########################################################
    now = time.process_time() # Only to show how long it takes to save
    ...
    print("It took " + str(time.process_time() - now) + " seconds to save channel 1 in binary format.")
    del now

    ## Read the NUMPY BINARY data back into python with:
    with open(filename, 'rb') as filehandle: # rb means open for reading binary
        ch1_npy = np.load(filehandle)
    del filename, filehandle
    print('Channel 1 Binary data has been recalled into "ch1_npy".\n')
...
if CHAN2_STATE == 1:
    ...
    with open(filename, 'wb') as filehandle: # wb means open for writing in binary; can overwrite
        ...
        ## NOTE, if one were to not use with open, and just do np.save like this:
            ## np.save(filename, np.vstack((DataTime,Data_Ch1)).T)
            ## this method automatically appends a .npy to the file name...
    print("It took " + str(time.process_time() - now) + " seconds to save channel 2 in binary format.")
    del now

    ## Read the NUMPY BINARY data back into python with:
    with open(filename, 'rb') as filehandle: # rb means open for reading binary
        ch2_npy = np.load(filehandle)
    del filename, filehandle
    print('Channel 2 Binary data has been recalled into "ch2_npy".\n')
...
font_size=14
ax=plt.subplot(211)
ax.plot(ch1_npy[:,0],ch1_npy[:,1],color='yellow')
ax.set_ylabel('Voltage (V)', fontsize=font_size)

plt.grid()

ax=plt.subplot(212)
plt.plot(ch2_npy[:,0],ch2_npy[:,1],color='green')
